...
public class SpreadSheetController {
   ...
   *
   * @param sheet      the model better spreadsheet.
   * @param readable   a readable input.
   * @param appendable an appendable output.
   */
  ...
  /**
   * Execution method for the controller.
   *
   * @throws IllegalStateException if message cannot be produced.
   */
        ...
        case "bulk-assign":
          try {
            ...
            System.out.println("Setting cells (" + row1 + "," + (col1 - 1)
                    + ") to (" + row2 + "," + (col2 - 1) + ")");
            sheet.setRectangle(row1, col1, row2, col2, value);
          } catch (IllegalArgumentException e) {
            writeMessage("Error: " + e.getMessage() + System.lineSeparator());
          }
          break;
